...
await build({ entryPoints: [path.join(root, 'apps/server/src/entrypoints/desktop-watchdog.ts')], bundle: true, platform: 'node', target: 'node22', format: 'cjs', outfile: path.join(output, 'server/desktop-watchdog.cjs'), banner: { js: 'const __localImportMetaUrl = require("node:url").pathToFileURL(__filename).href;' }, define: { 'import.meta.url': '__localImportMetaUrl' } });
