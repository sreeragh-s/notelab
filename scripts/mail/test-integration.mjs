...
if (!["localhost", "127.0.0.1", "[::1]"].includes(source.hostname)) throw new Error("Mail integration tests require a local PostgreSQL server.");
...
try {
  ...
  const exitCode = await new Promise((resolve, reject) => {
    const child = spawn("npm", ["test", "--workspace", "@zilobase/server", "--", "src/features/mail/mail.integration.test.ts"], {
      env: { ...process.env, MAIL_TEST_DATABASE_URL: source.toString() }, stdio: "inherit",
    });
