...
test.beforeEach(async ({ page }) => {
  await page.route("**/workspaces/workspace/calendar/**", async route => {
    ...
    if (url.pathname.endsWith("/ranges")) return route.fulfill({ json: { calendarId: "primary", start: url.searchParams.get("start"), end: url.searchParams.get("end"), generation: 1, revision: 1, events: [event], complete: true, nextPageToken: null } });
