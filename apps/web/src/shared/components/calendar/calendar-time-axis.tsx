...
export const TimeAxis = memo(function TimeAxis({ day, days, preferences }: { day: string; days: string[]; preferences: CalendarDisplayPreferences }) {
  ...
  return <div className="relative">{Array.from({ length: 24 }, (_, hour) => <div key={hour} style={{ height: preferences.hourHeight ?? 48 }} className="flex text-[10px] text-content-secondary">{zones.map(zone => <div className="w-14 pr-2 text-right" key={zone}>{eventClock({ dateTime: wallTime(day, `${String(hour).padStart(2, "0")}:00`, preferences.timeZone, "earlier"), timeZone: zone }, zone, preferences.timeFormat)}</div>)}</div>)}<CurrentTimeLabel hourHeight={preferences.hourHeight} days={days} zone={preferences.timeZone} secondaryZones={preferences.secondaryTimeZones} timeFormat={preferences.timeFormat} /></div>;
});
