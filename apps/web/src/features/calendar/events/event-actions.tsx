...
export function EventActions({ event, database, calendars, online, onEdit, onDuplicate, onDone, section = "manage" }: { event: CalendarEvent; database: CalendarDatabase; calendars: CalendarRecord[]; online: boolean; onEdit: () => void; onDuplicate: () => void; onDone: () => void; section?: "manage" | "rsvp" }) {
  ...
  if (section === "rsvp") return event.attendees.some(a => a.self) ? <div className="flex gap-1 rounded-md bg-surface-muted p-1" aria-label="Your response">{(["accepted", "declined", "tentative"] as const).map(status => <Button className="flex-1" key={status} variant={event.attendees.find(a => a.self)?.responseStatus === status ? "secondary" : "ghost"} aria-pressed={event.attendees.find(a => a.self)?.responseStatus === status} disabled={disabled} onClick={() => void act("rsvp", status)}>{status === "accepted" ? "Yes" : status === "declined" ? "No" : "Maybe"}</Button>)}</div> : null;
  return <div className="grid gap-2"><div className="flex flex-wrap gap-2">{writable && <><Button disabled={disabled} onClick={onEdit}>Edit</Button><Button variant="outline" disabled={disabled} onClick={onDuplicate}>Duplicate</Button><Button variant="ghost" disabled={disabled} onClick={() => setDeleting(true)}>Delete</Button></>}</div>
