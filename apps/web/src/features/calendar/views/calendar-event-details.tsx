...
function Section({ children }: { children: ReactNode }) { return <section className="grid gap-4 border-b border-stroke-default px-4 py-5 last:border-0">{children}</section>; }
function Row({ icon, children }: { icon: ReactNode; children: ReactNode }) { return <div className="flex items-start gap-3"><span className="mt-0.5 shrink-0 text-content-secondary [&>svg]:size-4">{icon}</span><div className="min-w-0 flex-1">{children}</div></div>; }
...
function Participant({ attendee }: { attendee: CalendarAttendee }) {
  ...
  return <div className="flex min-w-0 items-center gap-3"><Avatar size="sm"><AvatarFallback>{name.slice(0, 1).toUpperCase()}</AvatarFallback></Avatar><div className="min-w-0 flex-1"><p className="truncate" title={attendee.email}>{name}{attendee.self ? " (you)" : ""}</p><p className="text-xs text-content-secondary">{attendee.organizer ? "Organiser · " : ""}{responseLabel[attendee.responseStatus]}</p></div></div>;
}
export function CalendarEventDetails(props: Props) {
  ...
  return <div className="text-sm">
    <Section><h2 className="break-words text-lg font-medium">{event.title || "Untitled event"}</h2></Section>
    <Section>
      <Row icon={<Clock />}><div className="flex flex-wrap items-baseline gap-2"><span className="text-lg">{event.start.date ? "All day" : `${eventClock(event.start, props.zone, props.timeFormat)} → ${eventClock(event.end, props.zone, props.timeFormat)}`}</span>{!event.start.date && <span className="text-content-secondary">{duration}</span>}</div><p className="mt-2">{date}</p></Row>
    ...
    {Boolean(attendees.length || event.organizer) && <Section>
      <Row icon={<UsersIcon />}><h3 className="font-medium">{attendees.length} participants</h3><p className="mt-1 text-xs text-content-secondary">{attendees.filter(a => a.responseStatus === "accepted").length} yes · {attendees.filter(a => a.responseStatus === "declined").length} no · {attendees.filter(a => a.responseStatus === "tentative").length} maybe · {attendees.filter(a => a.responseStatus === "needsAction").length} awaiting</p></Row>
      {event.organizer && !attendees.some(a => a.email === event.organizer?.email) && <div><p className="truncate">{event.organizer.email}</p><p className="text-xs text-content-secondary">Organiser</p></div>}
      ...
      {others.length > 3 && <Button variant="ghost" className="justify-start text-content-secondary" aria-expanded={expanded} onClick={() => setExpanded(value => !value)}>{expanded ? "Show fewer participants" : `See all ${attendees.length} participants`}</Button>}
    ...
    {(event.conferenceUrl || event.location || event.description || event.conferenceStatus) && <Section>
      ...
      {event.conferenceUrl && /^https:\/\//.test(event.conferenceUrl) && <Row icon={<Video />}><a className="flex items-center gap-2 hover:underline" href={event.conferenceUrl} target="_blank" rel="noopener noreferrer">{event.conferenceUrl.startsWith("https://meet.google.com/") ? "Google Meet" : "Join meeting"}<ArrowUpRightIcon className="size-4" /></a></Row>}
    ...
    <Section>
      <Row icon={<CalendarIcon />}><p className="truncate">{calendar?.name || event.calendarId}</p><div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-content-secondary"><span>{event.transparency === "transparent" ? "Free" : "Busy"}</span><span>{event.visibility === "default" ? "Default visibility" : `${event.visibility[0]!.toUpperCase()}${event.visibility.slice(1)}`}</span></div></Row>
      <Row icon={<Bell />}><p className="text-content-secondary">Reminders</p>{reminders.length ? reminders.map((reminder, index) => <p key={index} className="mt-2">{reminder.minutes >= 60 && reminder.minutes % 60 === 0 ? `${reminder.minutes / 60}h` : `${reminder.minutes}min`} before{reminder.method === "email" ? " · Email" : ""}</p>) : <p className="mt-2">No reminders</p>}</Row>
