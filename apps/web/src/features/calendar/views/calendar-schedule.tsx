...
export function CalendarSchedule({ connections, userId, preferences: savedPreferences, preferenceWorkspaceId }: { preferenceWorkspaceId?: string; connections: CalendarConnection[]; userId: string; preferences: CalendarPreferences }) {
  ...
  const displayPreferences = useMemo(() => ({ ...preferences, prefetchLeadMs, bufferBefore: view === "month" ? buffers.monthBefore : buffers.before, bufferAfter: view === "month" ? buffers.monthAfter : buffers.after, visibleDayCount: search.days ?? 7, alignStart: search.align }), [prefetchLeadMs, preferences, search.days, search.align, view, buffers.before, buffers.after, buffers.monthBefore, buffers.monthAfter]);
  ...
  useEffect(() => { if (passiveDate.current === JSON.stringify([date, view, search.days, search.align, preferences.timeZone])) return; navigation.request(calendarDestinationRange({ date, view, days: search.days, align: search.align }, preferences), () => {}); }, [date, view, search.days, search.align, preferences.timeZone, navigation.request]);
