...
import { calendarDestinationRange, useCalendarNavigation, useCalendarVisibleDate } from "../workspace/calendar-navigation";
...
export function CalendarSchedule({ connections, userId, preferences: savedPreferences, preferenceWorkspaceId }: { preferenceWorkspaceId?: string; connections: CalendarConnection[]; userId: string; preferences: CalendarPreferences }) {
  ...
  const viewport = useCalendarVisibleDate(routeDate, search.date, workspace.travelZone ?? preferences.timeZone, workspace.visibleDate, workspace.setVisibleDate);
  const { anchorDate, date } = viewport;
  const { value: buffers } = useCalendarBuffers();
  const allDays = useMemo(() => calendarDays(anchorDate, view, preferences.weekStartsOn, search.days, preferences.showWeekends, search.align), [anchorDate, view, preferences.weekStartsOn, search.days, preferences.showWeekends, search.align]);
  ...
  const navigation = useCalendarNavigation({ ready: isRangeReady, online, error: data.find(d => d.error)?.error, initial: calendarDestinationRange({ date: anchorDate, view, days: search.days, align: search.align }, preferences), retry: () => data.forEach(d => d.refresh()) });
  useEffect(() => { requestScheduleRange(passiveDate, anchorDate, view, search, preferences, navigation.request); }, [anchorDate, view, search.days, search.align, preferences.timeZone, navigation.request]);
  useEffect(() => workspace.registerNavigation((next, commit) => navigation.request(calendarDestinationRange(next, preferences), () => {
  ...
  return <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
    ...
    {scheduleMainView({ query, connections, calendars, preferences, events, online, userId, open, emit: emitCalendarMetric, loadingMessage: scheduleLoadingMessage(online, data), isRangeReady, onViewportRange, navigation, passiveDate, markBookmark: viewport.markBookmark, view, search, navigate, savedPreferences, preferenceStore, workspace, date: anchorDate, headingDate: date, items, displayPreferences, setPeriod, setRange, selectItem, create, changeItem, onGeometryError })}
...
function scheduleMainView(props: { query: string; connections: CalendarConnection[]; calendars: CalendarRecord[]; preferences: CalendarPreferences; events: CalendarEvent[]; online: boolean; userId: string; open: (event: CalendarEvent) => void; emit: typeof emitCalendarMetric; loadingMessage?: string; isRangeReady: (range: CalendarRange) => boolean; onViewportRange: (range: CalendarRange) => void; navigation: ReturnType<typeof useCalendarNavigation>; passiveDate: { current: string | null }; markBookmark: (date: string) => void; view: CalendarView; search: { date?: string; days?: number; align?: boolean }; navigate: ReturnType<typeof useNavigate>; savedPreferences: CalendarPreferences; preferenceStore: ReturnType<typeof useCalendarPreferences>; workspace: ReturnType<typeof useCalendarWorkspace>; date: string; headingDate: string; items: CalendarItem[]; displayPreferences: CalendarDisplayPreferences; setPeriod: (next: string, view?: CalendarView) => void; setRange: (range: CalendarRange | null) => void; selectItem: (item: CalendarItem) => void; create: (day?: string, hour?: number, duration?: number, target?: CalendarRecord) => void; changeItem: (item: CalendarItem) => void; onGeometryError: (error: Error) => void }) {
  ...
  return <CalendarSurface onMetric={props.emit} loadingMessage={props.loadingMessage} isRangeReady={props.isRangeReady} onViewportRangeChange={props.onViewportRange} onRetryRange={props.navigation.retry} onPreviewDate={next => props.workspace.setVisibleDate(next)} onVisibleDateChange={next => bookmarkVisibleDate(props, next)} zoneControls={<CalendarTimeZones value={props.savedPreferences} onChange={value => props.preferenceStore.save.mutateAsync(value)} travelZone={props.workspace.travelZone} onRestore={() => props.workspace.setTravelZone(null)} date={new Date(`${props.headingDate}T12:00:00Z`)} />} items={props.items} date={props.date} view={props.view} preferences={props.displayPreferences} onNavigate={props.setPeriod} onRangeChange={props.setRange} onSelect={props.selectItem} onCreate={props.online ? props.create : undefined} onChange={props.changeItem} onError={props.onGeometryError} />;
}
function bookmarkVisibleDate(props: { view: CalendarView; search: { date?: string; days?: number; align?: boolean }; preferences: CalendarPreferences; passiveDate: { current: string | null }; markBookmark: (date: string) => void; navigate: ReturnType<typeof useNavigate> }, next: string) {
  props.markBookmark(next);
  props.passiveDate.current = JSON.stringify([next, props.view, props.search.days, props.search.align, props.preferences.timeZone]);
  if (props.search.date === next) return;
  void props.navigate({ to: "/calendar", replace: true, search: previous => ({ ...previous, date: next }) });
}
