...
export function CalendarSchedule({ connections, userId, preferences: savedPreferences, preferenceWorkspaceId }: { preferenceWorkspaceId?: string; connections: CalendarConnection[]; userId: string; preferences: CalendarPreferences }) {
  ...
  useEffect(() => { if (passiveDate.current === date) return; navigation.request(calendarDestinationRange({ date, view, days: search.days, align: search.align }, preferences), () => {}); }, [date, view, search.days, search.align, preferences.timeZone, navigation.request]);
  ...
  return <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
    {connections.map(c => <AccountData key={`${userId}:${c.workspaceId}:${c.bindingId}`} connection={c} userId={userId} start={cacheStart} end={cacheEnd} target={navigation.target ?? viewportRange} hiddenKeys={hiddenKeys} onData={onData} />)}
