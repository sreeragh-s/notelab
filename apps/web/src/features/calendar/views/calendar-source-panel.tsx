...
export function CalendarSourcePanel({ connection, calendar, allCalendars, userId, preferences, preferenceWorkspaceId, onSelect, onCreate }: {
  ...
  const events = (cache.events ?? []).filter(event => event.calendarId === calendar.id && event.status !== "cancelled" && (preferences.showDeclined || !event.attendees.some(attendee => attendee.self && attendee.responseStatus === "declined")))
