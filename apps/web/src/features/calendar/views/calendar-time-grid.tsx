...
function TimeAxis({ day, preferences }: { day: string; preferences: CalendarPreferences }) {
...
function TimeGridDay(props: GridProps & { day: string; axisWidth: number }) {
  ...
  const dayWidth = ((props.scrollRef.current?.clientWidth ?? 800) - props.axisWidth) / props.days.length;
...
export function CalendarTimeGrid(props: GridProps) {
  const { preferences, days } = props, axisWidth = 56 * (1 + preferences.secondaryTimeZones.length);
  const columns = `${axisWidth}px repeat(${days.length}, minmax(0, 1fr))`;
  return <div data-calendar-scroll ref={props.scrollRef} onScroll={event => props.onScroll?.(event.currentTarget.scrollTop)} className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-y-contain"><div className="min-w-0">
    <div className="sticky top-0 z-10 grid border-b border-stroke-default bg-surface-canvas" style={{ gridTemplateColumns: columns }}><div className="flex text-[10px] text-content-secondary">{[preferences.timeZone, ...preferences.secondaryTimeZones].map(zone => <span key={zone} title={zone} className="w-14 truncate p-1">{zone.split("/").at(-1)}</span>)}</div>{days.map(day => <div key={day} className="border-l border-stroke-default p-1"><Button size="sm" variant="ghost" className="w-full" onClick={() => props.onDay(day)}>{new Intl.DateTimeFormat(undefined, { weekday: "short", day: "numeric", timeZone: "UTC" }).format(new Date(`${day}T12:00:00Z`))}</Button>{(props.eventsByDay[day] ?? []).filter(event => event.start.date).map(event => <DraggableEvent key={calendarEventKey(event)} event={event} zone={preferences.timeZone} dayWidth={((props.scrollRef.current?.clientWidth ?? 800) - axisWidth) / days.length} disabled={!props.online || !props.writable(event)} onChange={props.onChange}>{props.card(event)}</DraggableEvent>)}</div>)}</div>
    <div className="grid" style={{ gridTemplateColumns: columns }}><TimeAxis day={days[0]!} preferences={preferences} />{days.map(day => <TimeGridDay key={day} {...props} day={day} axisWidth={axisWidth} />)}</div>
  </div></div>;
}
