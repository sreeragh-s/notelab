...
export function CalendarToolbar({ preferences, onSettings }: { preferences: CalendarPreferences; onSettings: () => void }) {
  ...
  const period = (date: string, view: CalendarView) => void navigate({ to: "/calendar", search: { date, view, days: search.days } });
  const today = () => period(todayInZone(preferences.timeZone), view);
  const shift = (direction: number) => period(shiftCalendarPeriod(date, view, direction, search.days, preferences.showWeekends), view);
  ...
  return <div className="flex min-w-0 items-center gap-1" aria-label="Calendar controls">
    ...
    {view === "week" && <Popover><PopoverTrigger asChild><Button variant="ghost" aria-label="Visible day count">{search.days ?? 7} days</Button></PopoverTrigger><PopoverContent className="w-56 p-3"><label className="text-sm" htmlFor="calendar-day-count">Visible days (1–31)</label><div className="mt-2 flex items-center gap-2"><Button variant="outline" aria-label="Fewer days" disabled={(search.days ?? 7) <= 1} onClick={() => void navigate({ to: "/calendar", search: { date, view, days: (search.days ?? 7) - 1 } })}>−</Button><Input id="calendar-day-count" type="number" min={1} max={31} value={search.days ?? 7} onChange={event => { const days = Number(event.target.value); if (Number.isInteger(days) && days >= 1 && days <= 31) void navigate({ to: "/calendar", search: { date, view, days } }); }} /><Button variant="outline" aria-label="More days" disabled={(search.days ?? 7) >= 31} onClick={() => void navigate({ to: "/calendar", search: { date, view, days: (search.days ?? 7) + 1 } })}>+</Button></div></PopoverContent></Popover>}
