...
export function CalendarAccountsSidebar({ workspaceId }: { workspaceId: string }) {
  ...
  return <>
    <aside className="space-y-4 px-2 py-2" aria-label="Calendar accounts">{preferences.query.data && <CalendarMiniCalendar preferences={preferences.query.data} />}{accounts.data?.connections.map((account, index) => <div key={account.bindingId} className="space-y-1">
      <div className="group/nav-row relative"><SidebarMenuButton asChild className={`${SIDEBAR_NAV_ROW_INTERACTION_CLASS_NAME} pr-8`}><h3 tabIndex={-1} id={`calendar-account-${account.bindingId}`} className="text-content-secondary"><span className="truncate" title={account.workspaceName}>{account.email}{account.workspaceId !== workspaceId && account.workspaceName ? ` · ${account.workspaceName}` : ""}</span></h3></SidebarMenuButton>
