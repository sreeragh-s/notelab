...
export function CalendarAccountsSidebar({ workspaceId }: { workspaceId: string }) {
  ...
  return <>
    <aside className="space-y-4 px-2 py-2" aria-label="Calendar accounts">{preferences.query.data && <CalendarMiniCalendar preferences={preferences.query.data} />}{orderedAccounts.map((account, index) => <div key={account.bindingId} className="space-y-1">
      <div className="group/nav-row relative"><SidebarMenuButton id={`calendar-account-${account.bindingId}`} disabled={!value || preferences.pending} aria-expanded={!value?.collapsedAccountIds?.includes(account.accountId)} onClick={() => { if (value) preferences.save.mutate({ ...value, collapsedAccountIds: value.collapsedAccountIds?.includes(account.accountId) ? value.collapsedAccountIds.filter(id => id !== account.accountId) : [...(value.collapsedAccountIds ?? []), account.accountId] }); }} className={`${SIDEBAR_NAV_ROW_INTERACTION_CLASS_NAME} pr-8 text-content-secondary`}><span className="truncate" title={account.workspaceName}>{account.email}{account.workspaceId !== workspaceId && account.workspaceName ? ` · ${account.workspaceName}` : ""}</span></SidebarMenuButton>
