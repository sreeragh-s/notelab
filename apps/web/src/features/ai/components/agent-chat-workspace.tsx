...
export function AgentChatWorkspace({
  ...
  const chat = isBootstrapping ? <LoadingChat /> : (
  ...
  const main = (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-surface-canvas">
      {isSidebar ? <ChatHeader onClose={onClose} onPresentationModeChange={onPresentationModeChange} presentationMode={presentationMode} /> : null}
      <div className="flex shrink-0 justify-end px-3 py-2">
        <Button aria-label="Ask AI settings" onClick={() => setSettings(!settingsOpen)} size="icon" type="button" variant="ghost">
          <SlidersHorizontalIcon className="size-4" />
        </Button>
      </div>
      <div className={isSidebar && settingsOpen
        ? "min-h-0 flex-1 overflow-hidden"
        : isSidebar
          ? "min-h-0 flex-1 overflow-y-auto px-4 py-4"
          : "min-h-0 flex-1 overflow-y-auto px-4 pb-5 md:px-6"} data-ai-scroll-shell>
...
function ChatHeader({ onClose, onPresentationModeChange, presentationMode }: {
  onClose?: () => void
  onPresentationModeChange?: (mode: ChatPresentationMode) => void
  presentationMode: ChatPresentationMode
}) {
  return (
    <header className="flex h-12 shrink-0 items-center gap-2 border-b px-3">
      {onClose ? <Button aria-label="Close Ask AI" onClick={onClose} size="icon-sm" type="button" variant="ghost">
        {presentationMode === "floating" ? <XIcon /> : <ChevronsRightIcon />}
      </Button> : null}
      <div className="min-w-0 flex-1"><h2 className="truncate font-heading text-sm font-medium">Ask AI</h2></div>
      {onPresentationModeChange ? <Button
