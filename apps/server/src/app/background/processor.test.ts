...
vi.mock("../../features/databases/automations/evaluator", () => ({ processDatabaseAutomationEventWindow: vi.fn() }));
vi.mock("../../features/databases/automations/run-engine", () => ({ processDatabaseAutomationRun: vi.fn() }));
vi.mock("../../features/mail/mail-index", () => ({ advanceMailIndex: fake.advance, publishMailIndexUpdate: fake.publish }));
vi.mock("../../features/mail/mail-database-sync-worker", () => ({ drainMailDatabaseSyncOutbox: fake.mail }));
