...
async function recoverInterruptedRestore(root: string) {
...
async function readLocalManifest(root: string) {
...
async function readLocalSecrets(root: string) {
...
async function initializePostgres(root: string, data: string, binary: (name: string) => string, secrets: { admin: string }) {
...
async function stopOrphanPostmaster(data: string, binary: (name: string) => string) {
...
async function waitForDatabase(adminUrl: string, exited: () => boolean) {
    let ready = false;
    for (let attempt = 0; attempt < 100; attempt++) {
      if (exited()) throw new Error("Local database exited during startup");
...
export async function startLocalDatabase(root: string, resources: string) {
  ...
  const manifest = await readLocalManifest(root);
  const secrets = await readLocalSecrets(root);
  const data = path.join(root, "postgres");
  const binary = (name: string) => path.join(resources, "postgres/bin", name);
  await initializePostgres(root, data, binary, secrets);
  await stopOrphanPostmaster(data, binary);
  ...
  child.once("exit", () => { if (initialized && !expectedStop) process.kill(process.pid, "SIGTERM"); });
  ...
  try {
    await waitForDatabase(adminUrl, () => Boolean(processError) || child.exitCode !== null || child.signalCode !== null);
