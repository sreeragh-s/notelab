...
export async function listAgentConversation(input: {
  ...
  const [conversation] = await db.select().from(aiAgentConversation).where(and(
    eq(aiAgentConversation.profileId, input.profileId),
    eq(aiAgentConversation.visibility, "shared"),
  )).limit(1);
  if (!conversation) throw new AgentProfileError("agent_conversation_not_found", "Agent conversation not found.", 404);
  const messages = await db.select().from(aiAgentConversationMessage)
...
export async function listLegacyAgentConversations(input: {
  ...
  await requireAgentProfileRole({ ...input, minimum: "user" });
  const conversations = await db.select().from(aiAgentConversation).where(and(
    eq(aiAgentConversation.profileId, input.profileId),
...
export async function submitAgentConversationMessage(input: {
  ...
  if ((intent === "configure" || intent === "configure_and_run") && role === "user") {
    throw new AgentProfileError("agent_configuration_forbidden", "Only agent editors can change its configuration.", 403);
  }
  if (input.clientId) {
    const [existing] = await db.select({ message: aiAgentConversationMessage })
      .from(aiAgentConversationMessage)
      .innerJoin(aiAgentConversation, eq(aiAgentConversation.id, aiAgentConversationMessage.conversationId))
      .where(and(
        eq(aiAgentConversation.profileId, input.profileId),
        eq(aiAgentConversation.visibility, "shared"),
    ...
    if (existing) {
      return { intent, message: existing.message, replayed: true, revision: null, run: null };
    }
  }
  ...
  if (intent === "run" || intent === "configure_and_run") {
...
export function classifyAgentMessage(message: string): CustomAgentChatIntent {
  ...
  const configure = /(?:\bconfigure\b|\bchange (?:the )?(?:agent|instructions|configuration|settings|model|name)\b|\bupdate (?:the )?(?:agent|instructions|configuration|settings|model|name)\b|\brename\b|\bset (?:the )?(?:name|instructions|model)\b|\binstructions?\s*:|\bfrom now on\b|\bevery (?:day|week|month|year)\b|\bschedule\b|\btrigger\b)/.test(value);
  const run = /(?:\brun\b|\bexecute\b|\btest (?:it|the agent)\b|\bdo it now\b|\bnow run\b|\buse (?:the )?.+ connector\b|\bcheck\b|\bfind\b|\bidentify\b|\bsummari[sz]e\b|\bcreate\b|\bupdate\b[^.!?\n]{0,80}\b(?:page|database)\b)/.test(value);
...
function inferDefinitionChange(
  ...
  const name = message.match(/(?:rename (?:this )?agent to|set (?:the )?name to|name this agent)\s+[“"']?([^\n“”"']+)/i)?.[1]?.trim();
  if (name) definition.name = name.slice(0, 120);
  const explicitInstructions = message.match(/instructions?\s*:\s*([\s\S]+)/i)?.[1]?.trim();
...
async function appendConversationMessage(input: {
  ...
  return db.transaction(async (tx) => {
    const [conversation] = await tx.select().from(aiAgentConversation).where(and(
      eq(aiAgentConversation.profileId, input.profileId),
      eq(aiAgentConversation.visibility, "shared"),
    )).limit(1).for("update");
    if (!conversation) throw new AgentProfileError("agent_conversation_not_found", "Agent conversation not found.", 404);
    if (input.clientId) {
      const [existing] = await tx.select().from(aiAgentConversationMessage).where(and(
    ...
    const [message] = await tx.insert(aiAgentConversationMessage).values({
    ...
    await tx.update(aiAgentConversation).set({
      lastActivityAt: now,
