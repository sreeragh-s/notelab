...
const triggerSchema = z.object({
  ...
  kind: z.enum(["manual", "schedule", "database", "comment", "mention", "meeting", "webhook", "slack", "connector"]),
  label: z.string().trim().min(1).max(120),
  status: z.enum(["active", "paused", "degraded", "disabled"]).optional(),
});
const runSchema = z.object({ prompt: z.string().trim().min(1).max(20_000).optional() });
...
aiAgentProfileRoutes.post("/agents/:agentId/duplicate", async (c) => handle(c, async (auth) => ({
  agent: await duplicateAgentProfile({ ...auth, profileId: c.req.param("agentId") }),
}), 201));

aiAgentProfileRoutes.get("/agents/:agentId/conversation", async (c) => handle(c, async (auth) => ({
  messages: await listAgentConversation({ ...auth, profileId: c.req.param("agentId") }),
})));

aiAgentProfileRoutes.get("/agents/:agentId/legacy-conversations", async (c) => handle(c, async (auth) => ({
  conversations: await listLegacyAgentConversations({ ...auth, profileId: c.req.param("agentId") }),
})));

aiAgentProfileRoutes.post("/agents/:agentId/conversation/messages", async (c) => handle(c, async (auth) => {
...
aiAgentProfileRoutes.get("/agents/:agentId/revisions", async (c) => handle(c, async (auth) => ({
  revisions: await listAgentRevisions({ ...auth, profileId: c.req.param("agentId") }),
})));

aiAgentProfileRoutes.post("/agents/:agentId/revisions/:revisionId/revert", async (c) => handle(c, async (auth) => ({
...
aiAgentProfileRoutes.get("/agents/:agentId/resources", async (c) => handle(c, async (auth) => ({
  resources: await listAgentResources({ ...auth, profileId: c.req.param("agentId") }),
})));

aiAgentProfileRoutes.put("/agents/:agentId/resources", async (c) => handle(c, async (auth) => {
  const body = resourceSchema.parse(await c.req.json());
  return { resources: await grantAgentResource({ ...auth, ...body, profileId: c.req.param("agentId") }) };
}));

aiAgentProfileRoutes.delete("/agents/:agentId/resources/:resourceId", async (c) => handle(c, async (auth) => {
  const resourceType = z.enum(["page", "database"]).parse(c.req.query("resourceType"));
  return { resources: await removeAgentResource({
    ...auth,
    profileId: c.req.param("agentId"),
...
aiAgentProfileRoutes.get("/agents/:agentId/triggers", async (c) => handle(c, async (auth) => ({
  triggers: await listAgentTriggers({ ...auth, profileId: c.req.param("agentId") }),
})));

aiAgentProfileRoutes.post("/agents/:agentId/triggers", async (c) => handle(c, async (auth) => {
  if (getStringEnv(c.env, "AI_CUSTOM_AGENT_TRIGGERS_ENABLED") !== "true") {
    throw new AgentProfileError("agent_triggers_disabled", "Custom Agent triggers are disabled.", 404);
  }
  const body = triggerSchema.parse(await c.req.json());
  return upsertAgentTrigger({ ...auth, ...body, env: c.env, profileId: c.req.param("agentId") });
}, 201));

aiAgentProfileRoutes.put("/agents/:agentId/triggers/:triggerId", async (c) => handle(c, async (auth) => {
  if (getStringEnv(c.env, "AI_CUSTOM_AGENT_TRIGGERS_ENABLED") !== "true") {
    throw new AgentProfileError("agent_triggers_disabled", "Custom Agent triggers are disabled.", 404);
  }
  const body = triggerSchema.parse(await c.req.json());
...
aiAgentProfileRoutes.delete("/agents/:agentId/triggers/:triggerId", async (c) => handle(c, async (auth) => (
  removeAgentTrigger({ ...auth, profileId: c.req.param("agentId"), triggerId: c.req.param("triggerId") })
)));

aiAgentProfileRoutes.post("/agents/:agentId/triggers/:triggerId/rotate-secret", async (c) => handle(c, async (auth) => (
  rotateAgentWebhookSecret({
    ...auth,
    env: c.env,
    profileId: c.req.param("agentId"),
...
aiAgentProfileRoutes.get("/agents/:agentId/runs", async (c) => handle(c, async (auth) => ({
  runs: await listAgentRuns({ ...auth, profileId: c.req.param("agentId") }),
})));

aiAgentProfileRoutes.post("/agents/:agentId/runs", async (c) => handle(c, async (auth) => {
  ...
  return { run: await startManualAgentRun({
    env: c.env,
    profileId,
    prompt: body.prompt,
    userId: auth.userId,
...
aiAgentProfileRoutes.get("/agents/:agentId/runs/:runId", async (c) => handle(c, async (auth) => (
  getAgentRunDetail({ ...auth, profileId: c.req.param("agentId"), runId: c.req.param("runId") })
)));

aiAgentProfileRoutes.get("/agents/:agentId/runs/:runId/approvals", async (c) => handle(c, async (auth) => {
  const profileId = c.req.param("agentId");
  ...
  }).from(aiAgentPendingAction).where(and(
  ...
  return { actions: actions.map((action) => ({ ...action, expiresAt: action.expiresAt.toISOString() })) };
}));

aiAgentProfileRoutes.post("/agents/:agentId/runs/:runId/cancel", async (c) => handle(c, async (auth) => ({
  run: await cancelAgentRun({ ...auth, profileId: c.req.param("agentId"), runId: c.req.param("runId") }),
})));

aiAgentProfileRoutes.post("/agents/:agentId/runs/:runId/actions/:actionId/approve", async (c) => handle(c, async (auth) => {
  const profileId = c.req.param("agentId");
  const runId = c.req.param("runId");
  await requireRunApprovalActor({ ...auth, profileId, runId });
  const now = new Date();
  const [action] = await db.update(aiAgentPendingAction).set({
    approvedAt: now,
    status: "executing",
    updatedAt: now,
  }).where(and(
  ...
  if (!action) throw new AgentProfileError("agent_approval_unavailable", "Approval is expired or was already handled.", 409);
  try {
...
aiAgentProfileRoutes.post("/agents/:agentId/runs/:runId/actions/:actionId/reject", async (c) => handle(c, async (auth) => {
  const profileId = c.req.param("agentId");
  const runId = c.req.param("runId");
  await requireRunApprovalActor({ ...auth, profileId, runId });
  const now = new Date();
  ...
  if (!action) throw new AgentProfileError("agent_approval_unavailable", "Approval was already handled.", 409);
  ...
  await db.update(aiAgentConversationMessage).set({
    parts: [{ status: "failed", text: "Run stopped because a required action was rejected.", type: "run" }],
    status: "failed",
    updatedAt: now,
  }).where(eq(aiAgentConversationMessage.runId, runId));
  await appendRunEvent(runId, "approval_rejected", "shared", { actionId: action.id, actorUserId: auth.userId });
...
async function requireRunApprovalActor(input: {
  ...
  const [run] = await db.select({ initiatedByUserId: aiAgentRun.initiatedByUserId })
    .from(aiAgentRun).where(and(
  ...
  if (!allowed) throw new AgentProfileError("agent_approval_forbidden", "You cannot approve this agent run.", 403);
}
...
async function handle(
  ...
  if (getStringEnv(c.env, "AI_CUSTOM_AGENTS_ENABLED") !== "true") {
    return c.json({ code: "AI_CUSTOM_AGENTS_DISABLED", error: "Custom Agents are disabled." }, 404);
  }
  try {
