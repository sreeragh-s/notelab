...
`dev:clipper` is not the Zilobase API. It starts WXT’s hot-reload server on `http://localhost:3400` and opens Chrome with the extension loaded. Keep `npm run dev` running separately; the clipper still saves to `http://localhost:3000`.
