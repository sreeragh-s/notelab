...
Release validation added an all-window save barrier before maintenance and Quit. A failed/timed-out acknowledgement stops maintenance. Runtime reopening reloads other local windows, and health probes can request up to three backend restarts within five minutes while installation locks suppress recovery during maintenance or explicit remote switching. The native navigation plugin rejects external WebView navigations in local mode. User-requested remote profile verification first suspends local jobs; failed verification permits local recovery. The [onboarding runbook](../../docs/desktop/local-support.md) describes the supported choices and boundaries.
