...
[Health readiness](../../../apps/server/src/features/health/readiness.ts) checks database and object storage through controlled dependencies and reads realtime/background readiness. [Image routes](../../../apps/server/src/features/images/routes.ts) own upload/body/complete/read/delete transport and page access; [image input rules](../../../apps/server/src/features/images/image-upload-input.ts) own numeric/MIME/filename/object-key normalization. Storage variation remains in infrastructure.

[Metadata routes](../../../apps/server/src/features/metadata/routes.ts) own authentication, query validation and status mapping. [Bookmark metadata](../../../apps/server/src/features/metadata/bookmark-metadata.ts) owns hostname filtering, fetching, bounded HTML reading and metadata precedence. It retains the existing follow-redirect behavior and input hostname checks; this extraction does not add DNS or redirect-target validation.
