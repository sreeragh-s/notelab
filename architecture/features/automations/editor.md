...
[DatabaseAutomationManager](../../../apps/web/src/features/databases/automations/database-automation-manager.tsx) is the browser entrypoint. It renders list/history navigation, the editing dialog and discard confirmation. [Manager state](../../../apps/web/src/features/databases/automations/use-automation-manager.ts) owns selected automation/run IDs, the editing baseline, query subscriptions, validation and save coordination. The existing [manager screens](../../../apps/web/src/features/databases/automations/database-automation-screens.tsx) remain a small, cohesive set of list and run-history presentations.

[Draft rules](../../../apps/web/src/features/databases/automations/definition/automation-draft.ts) convert editor state to definitions and back, preserve typed trigger operands and derive names. [Schedule rules](../../../apps/web/src/features/databases/automations/definition/schedule-model.ts) own schedule serialization and labels. These modules have no runtime React or presentation dependency. [Definition builder](../../../apps/web/src/features/databases/automations/definition/automation-builder.tsx), [trigger controls](../../../apps/web/src/features/databases/automations/definition/automation-trigger-controls.tsx) and [property-action controls](../../../apps/web/src/features/databases/automations/actions/property-action-controls.tsx) own their editing interactions. [Property-action rules](../../../apps/web/src/features/databases/automations/actions/property-action-model.ts) convert values and label compact actions. Trigger and action pickers reuse the same [picker controls](../../../apps/web/src/features/databases/automations/automation-picker-controls.tsx); their different operator/value flows remain separate.
...
Save first [materializes new webhook header secrets](../../../apps/web/src/features/databases/automations/definition/materialize-webhook-secrets.ts) sequentially through the existing secret mutation, then rebuilds the definition with secret IDs. It updates an existing automation with the loaded version or creates a new one. The baseline and screen change only after the save resolves. Shared [React bindings](../../../packages/features/src/databases/automations/react) retain query keys and invalidation policies. The definition never receives plaintext draft header values.
