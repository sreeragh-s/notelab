...
The Calendar schedule composes authorized cache subscriptions into a continuous timeline. [Timed views](../../../apps/web/src/shared/components/calendar/calendar-timeline.tsx) use independent civil-date keys and one native two-axis scroll owner. Sticky date/all-day headers and timezone rails share the event coordinate plane. Visible-day count controls column width. Chrome follows the leftmost visible date while scrolling; after inertia stops the scroller aligns to the nearest civil-date column once and bookmarks the route. The expanded all-day band is 96px and collapsed band 24px, with overflow scrolling.
