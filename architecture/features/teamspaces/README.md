...
In local mode, new teamspaces use private access with guests/public sharing disabled. The management service rejects collaboration policy fields embedded in ordinary updates; the transport guard returns `FEATURE_UNAVAILABLE_LOCAL` before such payloads reach handlers. Names, icons and descriptions remain editable.
