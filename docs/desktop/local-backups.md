...
Backup requests save acknowledgements from all open windows (and stops if a window fails or times out), flushes the active editor, ends native recording and stops the API before dumping and copying. Each payload has a SHA-256 checksum; creation extracts and verifies the temporary archive before atomic publication. A failed backup keeps the previous archive. The runtime reopens afterward. The scheduler checks daily while the app is running, defers during recording and retains seven dated daily archives. It skips unchanged installations using persisted database mutation counters and object/recovery metadata; session-only changes are excluded. Counter resets conservatively cause a new backup.
...
Validation: `node scripts/desktop/local/backup-smoke.mjs` exercises packaged backup and fresh-install restore. Archive tests cover payload integrity, truncation and unsafe names. Multi-window runtime behaviour, low-disk interruption and clean-machine native dialog testing remain release acceptance work.
